        ...
        for rec in self:
            ...
            if rec.send_as_recipient:
                for partner in self.to_partner_ids:
                    ...
                    template.with_context(user=user).sudo(user=user).send_mail(rec.id, force_send=False, raise_exception=True, email_values={'recipient_ids': [(4, pid) for pid in partner.ids]})
            else:
               template.with_context(user=rec.send_as_user_id).sudo(user=rec.send_as_user_id).send_mail(rec.id, force_send=False, raise_exception=True, email_values={'recipient_ids': [(4, pid) for pid in rec.to_partner_ids.ids]}) #, email_values={'email_to': user.email, 'subject': subject})
