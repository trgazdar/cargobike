    ...
    def _create_stock_moves(self, picking):
        ...
        for key, lines in date_groups:
            ...
            if picking:
                ...
                location = self.env['stock.location'].browse(copy_vals.get('location_dest_id'))
                warehouse = self.env['stock.warehouse'].search([('view_location_id', '=', location.location_id.id)])
                picking_type = self.env['stock.picking.type'].search(
                    [('warehouse_id', '=', warehouse.id), ('code', '=', 'incoming')], limit=1).id
                copy_vals.update({'picking_type_id': picking_type})
