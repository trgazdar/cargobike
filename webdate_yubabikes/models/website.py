		...
		if vals.get("product"):
			...
			query = """WITH forecast_qty AS (
										...
										SELECT
											...
											sum(product_qty) as product_qty
										FROM forecast_qty where product_id = %(product)s and state = 'forecast'
										GROUP BY product_id, state, date, company_id, warehouse_id order by date asc)t  where CASE WHEN SIGN(product_qty) > 0 THEN 1 else 0 END > 0 and date >= current_date GROUP BY date, myqt order by mdate desc limit 1"""
			params = {
				'product': product}
			self.env.cr.execute(query, params)
