...
class AccountFiscalPosition(models.Model):
    ...
    def _get_fpos_by_region(self, country_id=False, state_id=False, zipcode=False,
        ...
        fiscal_position = self.search(domain + [('country_id', '=', origin_country_ept)], limit=1)
        if fiscal_position:
            return fiscal_position
        fiscal_position = self.search(domain + [('country_group_id.country_ids', '=', origin_country_ept)],
                                      limit=1)
        if fiscal_position:
            return fiscal_position
