    ...
    def compute_initial_final_date(self):
        ...
        for statement in self:
            if statement.line_ids:
                ...
                statement.initial_date = statement.date
                statement.final_date = statement.date
...
class AccountBankStatementLine(models.Model):
    ...
    @api.depends('amount', 'journal_id', 'date')
    def _compute_amount_main_currency(self):
        for item in self:
            if item.statement_id:
                company_id = item.statement_id.company_id
                main_currency = item.company_id.currency_id
                line_currency = item.statement_id.currency_id
                item.amount_main_currency = line_currency._convert(
                    item.amount, main_currency, company_id, item.date)
            else:
                item.amount_main_currency = item.amount
