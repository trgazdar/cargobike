odoo.define('website_sale.website_sale_minimum', function (require) {
'use strict';
...
sAnimations.registry.WebsiteSale = sAnimations.Class.extend(ProductConfiguratorMixin, {
    ...
    _changeCartQuantity: function ($input, value, $dom_optional, line_id, productIDs) {
        ...
        this._rpc({
            ...
            if (data.cart_quantity) {
                // console.log("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZz",data.cart_quantity)
    ...
    _getProductId: function ($parent) {
        if ($parent.find('input.js_product_change').length !== 0) {
            console.log("WWWWWWWWWWWWWWww",parseInt($parent.find('input.js_product_change:checked').val()))
    ...
    _onChangeCartQuantity: function (ev) {
        ...
        var product_qty = $('.product_min_qty').val()
        var qty_new = $(".js_quantity").val();
        var min_qty = parseInt(product_qty)
        var new_qty = parseInt(qty_new)
        rpc.query({
        ...
        .then(function(qty){
            var js_value = $input.data('js_quantity')
            if ($input.val() < qty){
                alert("Quantity should not be less than Minimum Quantity of the Product")
                $input.val(qty)
            }
        });
